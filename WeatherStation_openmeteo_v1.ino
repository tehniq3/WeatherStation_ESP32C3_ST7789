 ...
 * ==============================================================================
 * SOLUȚIA PENTRU CRASH-UL DE MEMORIE RAM DE PE ESP32-C3 (Inspirată din proiectul TM1637)
 * ==============================================================================
 * Problema inițială: Variabila `WiFiClientSecure client` era declarată GLOBAL.
 * Pe ESP32-C3, HTTPS-ul consumă fix ~20KB de RAM pentru buffer-ul de criptare.
 * Adăugând biblioteca TFT_eSPI, RAM-ul s-a umplut, iar funcția http.getString() 
 * eșua returnând un string gol (Eroare JSON: InvalidInput).
 * 
 * Soluția aplicată aici:
 * 1. `WiFiClientSecure client` a fost mutată LOCAL în interiorul funcției de rețea.
 *    Astfel, cei 20KB se alocază DOAR în momentul descărcării și se ELIBEREAZĂ instant.
 * 2. Funcția drawUI() este apelată DOAR dacă parsarea JSON a reușit cu adevărat.
 * ==============================================================================
 */
...
// ATENȚIE: Nu mai declarăm "WiFiClientSecure client;" aici global!
...
void updateWeatherAndForecast() {
  // CLIENT LOCAL: Se distruge la sfârșitul funcției și eliberează RAM-ul!
  WiFiClientSecure client;
  client.setInsecure(); 

  HTTPClient http;
  String url = "https://api.open-meteo.com/v1/forecast?latitude=44.33&longitude=23.79&current=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m,surface_pressure&daily=weather_code,temperature_2m_max&timezone=auto&forecast_days=5";
  ...
  if (http.begin(client, url)) {
    ...
    if (httpCode == HTTP_CODE_OK) {
      ...
      if (error) {
        ...
        Serial.printf("[DEBUG] Temp: %.1f, Wind: %.1f, Pres: %.0f\n", lastTemp, lastWs, lastPres);
    ...
    http.end();
  } // Aici "client" este șters din memorie
}
